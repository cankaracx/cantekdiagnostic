drop function if exists public.search_manual_chunks(
  extensions.vector,
  text,
  boolean,
  integer
...
as $$
  with query_values as (
    select
      plainto_tsquery('simple', left(coalesce(query_text, ''), 1000)) as tsq,
      nullif(btrim(left(coalesce(equipment_filter, ''), 80)), '') as equipment_needle
  ),
  ...
  select
    ...
    (
      ...
      + case
          when query_values.equipment_needle is not null
            and document.equipment ilike '%' || query_values.equipment_needle || '%'
          then 1.8
          else 0
        end
